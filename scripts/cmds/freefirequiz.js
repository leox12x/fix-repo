...
module.exports = {
  ...
  onStart: async function ({ api, event, args, usersData }) {
    ...
    if (!userData.data?.ffAttempts || userData.data.ffAttempts.lastReset !== today) {
      userData.data.ffAttempts = { count: 0, lastReset: today };
    ...
    try {
      const listRes = await axios.get("https://mahmud-infinity-api.onrender.com/api/game/ff?list=true");
      apiStats = listRes.data || [];
      const userRes = await axios.get(`https://mahmud-infinity-api.onrender.com/api/game/ff?userID=${senderID}`);
      currentUserStats = userRes.data || { win: 0, loss: 0 };
    } catch (e) {}

    if (args[0] === "list") {
      if (!apiStats.length) return api.sendMessage("❌ | No rankings yet.", threadID, messageID);
      let msg = `👑 Free Fire Quiz Global Ranking:\n\n`;
    ...
    const drawRankCard = async () => {
      ...
      const dailyDone = userData.data.ffAttempts.count || 0;
      ...
      ctx.fillText("Free Fire Quiz Personal Analytics", 135, 76);
      ...
      // Featured Image with Stroke
      drawPanel(ctx, 340, 115, 300, 440, "FEATURED CHARACTER");
      try {
        const midImg = await loadImage("https://i.imgur.com/Kc1SRtW.png");
      ...
      // Top 10 Masters
      drawPanel(ctx, 660, 115, 320, 440, "TOP 10 FF MASTERS");
      let startY = 185;
      for (let i = 0; i < 10; i++) {
        const p = apiStats[i];
      ...
      const cachePath = path.join(process.cwd(), "cache", `ffqz_rank_${senderID}.png`);
    ...
    if (args[0] === "info" || args[0] === "rank") {
      ...
      return api.sendMessage({ body: `📊 Free Fire Quiz info for ${userData.name}`, attachment: fs.createReadStream(cachePath) }, threadID, () => { if (fs.existsSync(cachePath)) fs.unlinkSync(cachePath); }, messageID);
    }

    if (userData.data.ffAttempts.count >= maxLimit) {
      const cachePath = await drawRankCard();
      return api.sendMessage({ body: `• 𝐁𝐚𝐛𝐲, 𝐲𝐨𝐮 𝐡𝐚𝐯𝐞 𝐫𝐞𝐚𝐜𝐡𝐞𝐝 𝐲𝐨𝐮𝐫 𝐅𝐅 𝐐𝐮𝐢𝐳 𝐥𝐢𝐦𝐢𝐭!`, attachment: fs.createReadStream(cachePath) }, threadID, () => { if (fs.existsSync(cachePath)) fs.unlinkSync(cachePath); }, messageID);
    }

    // --- Game Logic ---
    try {
      const apiUrl = await baseApiUrl();
      const response = await axios.get(`${apiUrl}/api/freefire`);
      const { name, imgurLink } = response.data.freefire;
      const imgStream = await axios({ url: imgurLink, method: "GET", responseType: "stream", headers: { "User-Agent": "Mozilla/5.0" } });
      
      userData.data.ffAttempts.count++;
      await usersData.set(senderID, userData);

      api.sendMessage({ body: `• 𝐀 𝐑𝐚𝐧𝐝𝐨𝐦 𝐅𝐫𝐞𝐞 𝐅𝐢𝐫𝐞 𝐜𝐡𝐚𝐫𝐚𝐜𝐭𝐞𝐫 𝐚𝐩𝐩𝐞𝐚𝐫𝐞𝐝!\n𝐆𝐮𝐞𝐬𝐬 𝐭𝐡𝐞 𝐜𝐡𝐚𝐫𝐚𝐜𝐭𝐞𝐫 𝐧𝐚𝐦𝐞.\n\n• 𝐃𝐚𝐢𝐥𝐲 𝐋𝐢𝐦𝐢𝐭: ${userData.data.ffAttempts.count}/${maxLimit}`, attachment: imgStream.data }, threadID, (err, info) => {
        if (err) return;
        global.GoatBot.onReply.set(info.messageID, { commandName: this.config.name, type: "reply", messageID: info.messageID, author: senderID, correctName: name, answered: false });
        ...
        setTimeout(async () => {
          ...
          if (session && !session.answered) {
            ...
            await axios.post("https://mahmud-infinity-api.onrender.com/api/game/ff/loss", { userID: senderID });
    ...
    } catch (e) { api.sendMessage("❌ Failed to start FF quiz.", threadID, messageID); }
  },

  onReply: async function ({ api, event, Reply, usersData }) {
    const { correctName, author, messageID, answered } = Reply;
    const { senderID, threadID, messageID: replyMsgID } = event;

    if (senderID !== author) return api.sendMessage("• 𝐁𝐚𝐛𝐲, 𝐧𝐨𝐭 𝐲𝐨𝐮𝐫 𝐭𝐮𝐫𝐧\n𝐓𝐲𝐩𝐞 !𝐟𝐟𝐪𝐳 𝐭𝐨 𝐬𝐭𝐚𝐫𝐭 𝐲𝐨𝐮𝐫 𝐆𝐚𝐦𝐞.", threadID, replyMsgID);
    if (answered) return;

    Reply.answered = true;
    const isCorrect = correctName.toLowerCase() === event.body.trim().toLowerCase();
    let userData = await usersData.get(senderID);

    if (isCorrect) {
      userData.money = (userData.money || 0) + 1000;
      userData.exp = (userData.exp || 0) + 121;
      await axios.post("https://mahmud-infinity-api.onrender.com/api/game/ff/win", { userID: senderID });
