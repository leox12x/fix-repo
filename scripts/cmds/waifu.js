...
module.exports = {
  ...
  onStart: async function ({ api, event, args, usersData }) {
    ...
    const maxLimit = 10;
    // Defined timeLeft here so it's accessible everywhere in onStart
    const timeLeft = moment.duration(moment.tz("Asia/Dhaka").endOf('day').diff(now));

    if (!userData.data?.waifu || userData.data.waifu.lastReset !== today) {
      userData.data.waifu = { count: 0, lastReset: today };
      await usersData.set(senderID, userData);
    }

    // --- Global API Data Fetching (Waifu Stats) ---
    let apiStats = [], currentUserStats = { win: 0, loss: 0 };
    try {
      const listRes = await axios.get("https://mahmud-infinity-api.onrender.com/api/game/waifu?list=true");
      apiStats = listRes.data || [];
      const userRes = await axios.get(`https://mahmud-infinity-api.onrender.com/api/game/waifu?userID=${senderID}`);
      currentUserStats = userRes.data || { win: 0, loss: 0 };
    } catch (e) {}

    if (args[0] === "list") {
      if (!apiStats.length) return api.sendMessage("❌ | No rankings yet.", threadID, messageID);
      let msg = `👑 Waifu Quiz Global Ranking:\n\n`;
    ...
    const drawRankCard = async () => {
      ...
      const dailyDone = userData.data.waifu.count || 0;
      ...
      ctx.fillStyle = "rgba(255, 255, 255, 0.6)"; ctx.font = "bold 16px Arial"; ctx.fillText("Waifu Quiz Personal Analytics", 135, 76);
      ...
      // Panel 2: Featured
      drawPanel(ctx, 340, 115, 300, 440, "FEATURED WAIFU");
      try {
        const midImg = await loadImage("https://i.imgur.com/LlzXnEZ.jpeg");
      ...
      // Panel 3: Ranking
      drawPanel(ctx, 660, 115, 320, 440, "TOP 10 WAIFU RANKING");
      let startY = 185;
      for (let i = 0; i < 10; i++) {
        const p = apiStats[i];
      ...
      const cachePath = path.join(process.cwd(), "cache", `waifu_rank_${senderID}.png`);
    ...
    if (args[0] === "info" || args[0] === "rank") {
      ...
      return api.sendMessage({ body: `📊 Waifu Quiz info for ${userData.name}`, attachment: fs.createReadStream(cachePath) }, threadID, () => { if (fs.existsSync(cachePath)) fs.unlinkSync(cachePath); }, messageID);
    }

    if (userData.data.waifu.count >= maxLimit) {
      const cachePath = await drawRankCard();
      // Fixed: duration changed to timeLeft
      return api.sendMessage({ body: `• 𝐁𝐚𝐛𝐲, 𝐲𝐨𝐮 𝐡𝐚𝐯𝐞 𝐫𝐞𝐚𝐜𝐡𝐞𝐝 𝐲𝐨𝐮𝐫 𝐖𝐚𝐢𝐟𝐮 𝐥𝐢𝐦𝐢𝐭!\n• 𝐓𝐫𝐲 𝐚𝐠𝐚𝐢𝐧 𝐢𝐧 ${timeLeft.hours()}𝐡 ${timeLeft.minutes()}𝐦 𝐥𝐚𝐭𝐞𝐫.`, attachment: fs.createReadStream(cachePath) }, threadID, () => { if (fs.existsSync(cachePath)) fs.unlinkSync(cachePath); }, messageID);
    }
    ...
    try {
      ...
      const waifuNames = Array.isArray(name) ? name : [name];
      const imgStream = await axios({ url: imgurLink, method: "GET", responseType: "stream", headers: { "User-Agent": "Mozilla/5.0" } });
      
      userData.data.waifu.count++;
      await usersData.set(senderID, userData);

      api.sendMessage({ body: `• A waifu appeared! Guess her name.\n\n• 𝐃𝐚𝐢𝐥𝐲 𝐋𝐢𝐦𝐢𝐭: ${userData.data.waifu.count}/${maxLimit}`, attachment: imgStream.data }, threadID, (err, info) => {
        if (err) return;
        global.GoatBot.onReply.set(info.messageID, { commandName: this.config.name, type: "reply", messageID: info.messageID, author: senderID, waifuNames, answered: false });
