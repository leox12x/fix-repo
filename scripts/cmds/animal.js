...
module.exports = {
  ...
  onStart: async function ({ api, event, args, usersData }) {
    ...
    if (!userData.data?.animalAttempts || userData.data.animalAttempts.lastReset !== today) {
      userData.data.animalAttempts = { count: 0, lastReset: today };
      await usersData.set(senderID, userData);
    }

    // --- Global API Data Fetching (Animal Stats) ---
    let apiStats = [], currentUserStats = { win: 0, loss: 0 };
    try {
      const listRes = await axios.get("https://mahmud-infinity-api.onrender.com/api/game/animal?list=true");
      apiStats = listRes.data || [];
      const userRes = await axios.get(`https://mahmud-infinity-api.onrender.com/api/game/animal?userID=${senderID}`);
      currentUserStats = userRes.data || { win: 0, loss: 0 };
    } catch (e) {}

    if (args[0] === "list") {
      if (!apiStats.length) return api.sendMessage("❌ | No rankings yet.", threadID, messageID);
      let msg = `👑 Animal game Global Ranking:\n\n`;
    ...
    const drawRankCard = async () => {
      ...
      ctx.fillStyle = "rgba(255, 255, 255, 0.6)"; ctx.font = "bold 16px Arial"; ctx.fillText("Animal Quiz Personal Analytics", 135, 76);
      ...
      // Featured Animal Panel
      drawPanel(ctx, 340, 115, 300, 440, "FEATURED ANIMAL");
      try {
        const midImg = await loadImage("https://i.imgur.com/j1qRIhw.png");
      ...
      // Top 10 Hunters
      drawPanel(ctx, 660, 115, 320, 440, "TOP 10 ANIMAL HUNTERS");
      let startY = 185;
      for (let i = 0; i < 10; i++) {
        const p = apiStats[i];
      ...
      const cachePath = path.join(process.cwd(), "cache", `animal_rank_${senderID}.png`);
    ...
    if (args[0] === "info" || args[0] === "rank") {
      ...
      return api.sendMessage({ body: `📊 Animal Quiz info for ${userData.name}`, attachment: fs.createReadStream(cachePath) }, threadID, () => { if (fs.existsSync(cachePath)) fs.unlinkSync(cachePath); }, messageID);
    }

    if (userData.data.animalAttempts.count >= maxLimit) {
      const cachePath = await drawRankCard();
      return api.sendMessage({ body: `• 𝐁𝐚𝐛𝐲, 𝐲𝐨𝐮 𝐡𝐚𝐯𝐞 𝐫𝐞𝐚𝐜𝐡𝐞𝐝 𝐲𝐨𝐮𝐫 𝐀𝐧𝐢𝐦𝐚𝐥 𝐥𝐢𝐦𝐢𝐭!`, attachment: fs.createReadStream(cachePath) }, threadID, () => { if (fs.existsSync(cachePath)) fs.unlinkSync(cachePath); }, messageID);
    }
    ...
    try {
      ...
      userData.data.animalAttempts.count++;
      await usersData.set(senderID, userData);

      api.sendMessage({ body: `• 𝐆𝐮𝐞𝐬𝐬 𝐭𝐡𝐞 𝐚𝐧𝐢𝐦𝐚𝐥 𝐧𝐚𝐦𝐞 𝐢𝐧 𝐭𝐡𝐞 𝐢𝐦𝐚𝐠𝐞!\n\n• 𝐃𝐚𝐢𝐥𝐲 𝐋𝐢𝐦𝐢𝐭: ${userData.data.animalAttempts.count}/${maxLimit}`, attachment: imgStream.data }, threadID, (err, info) => {
        if (err) return;
        global.GoatBot.onReply.set(info.messageID, { commandName: this.config.name, type: "reply", messageID: info.messageID, author: senderID, animalNames, answered: false });
