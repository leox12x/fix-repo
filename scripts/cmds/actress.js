...
module.exports = {
  ...
  onStart: async function ({ api, event, args, usersData }) {
    ...
    if (!userData.data.actressAttempts || userData.data.actressAttempts.lastReset !== today) {
      userData.data.actressAttempts = { count: 0, lastReset: today };
    ...
    if (args[0] === "list" || args[0] === "rank") {
      try {
        const listRes = await axios.get("https://mahmud-infinity-api.onrender.com/api/game/actress?list=true");
        const apiStats = listRes.data || [];
        
        if (args[0] === "list") {
          let msg = `👑 Actress Quiz Ranking:\n\n`;
          ...
          const userRes = await axios.get(`https://mahmud-infinity-api.onrender.com/api/game/actress?userID=${senderID}`);
          const stats = userRes.data || { win: 0, loss: 0 };
          
          let infoMsg = `📊 𝐀𝐜𝐭𝐫𝐞𝐬𝐬 𝐒𝐭𝐚𝐭𝐢𝐬𝐭𝐢𝐜𝐬 𝐟𝐨𝐫 ${userData.name}\n${"━".repeat(20)}\n`;
          infoMsg += `• Global Rank: #${index === -1 ? "N/A" : index + 1}\n`;
          infoMsg += `• Total Wins: ${stats.win}\n• Total Lost: ${stats.loss}\n`;
          infoMsg += `• Daily: ${userData.data.actressAttempts.count}/${maxLimit}\n${"━".repeat(20)}`;
          return api.sendMessage(infoMsg, threadID, messageID);
        }
      } catch (e) { return api.sendMessage("❌ Error fetching stats.", threadID, messageID); }
    }

    if (userData.data.actressAttempts.count >= maxLimit) {
      const duration = moment.duration(moment.tz("Asia/Dhaka").endOf('day').diff(now));
      return api.sendMessage(`• 𝐁𝐚𝐛𝐲, 𝐲𝐨𝐮 𝐡𝐚𝐯𝐞 𝐫𝐞𝐚𝐜𝐡𝐞𝐝 𝐲𝐨𝐮𝐫 𝐀𝐜𝐭𝐫𝐞𝐬𝐬 𝐥𝐢𝐦𝐢𝐭, 𝐭𝐫𝐲 𝐚𝐠𝐚𝐢𝐧 𝐢𝐧 ${duration.hours()}𝐡 ${duration.minutes()}𝐦.`, threadID, messageID);
    }

    try {
      const apiUrl = await baseApiUrl();
      const response = await axios.get(`${apiUrl}/api/actress`);
      const { name, imgurLink } = response.data.actress;
      const actressNames = Array.isArray(name) ? name : [name];
      ...
      userData.data.actressAttempts.count++;
      await usersData.set(senderID, userData);

      api.sendMessage({
        body: "• Guess the actress:",
        attachment: imageStream.data
      }, threadID, (err, info) => {
        if (err) return;
        global.GoatBot.onReply.set(info.messageID, { commandName: this.config.name, author: senderID, actressNames, answered: false });
        ...
        setTimeout(async () => {
          ...
          if (session && !session.answered) {
            ...
            await axios.post("https://mahmud-infinity-api.onrender.com/api/game/actress/loss", { userID: senderID });
            await usersData.set(senderID, uData);
            api.editMessage(`• 𝐓𝐢𝐦𝐞 𝐨𝐯𝐞𝐫 𝐁𝐚𝐛𝐲! 🥹\nCorrect Answer: ${actressNames[0]}\nYou lost 300 coins & 121 exp.`, info.messageID);
  ...
  onReply: async function ({ api, event, Reply, usersData }) {
    const { actressNames, author, answered } = Reply;
    ...
    const isCorrect = actressNames.some(n => n.toLowerCase() === body.trim().toLowerCase());
    let userData = await usersData.get(senderID);

    if (isCorrect) {
      userData.money += 1000; userData.exp += 121;
      await axios.post("https://mahmud-infinity-api.onrender.com/api/game/actress/win", { userID: senderID });
