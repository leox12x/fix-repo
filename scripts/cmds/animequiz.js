...
module.exports = {
  ...
  onStart: async function ({ api, event, args, usersData }) {
    ...
    if (!userData.data?.animes || userData.data.animes.lastReset !== today) {
      userData.data.animes = { count: 0, lastReset: today };
      await usersData.set(senderID, userData);
    }

    // --- API Data Fetching for Stats & Leaderboard ---
    let apiStats = [], currentUserStats = { win: 0, loss: 0 };
    try {
      const listRes = await axios.get("https://mahmud-infinity-api.onrender.com/api/game/aniqz?list=true");
      apiStats = listRes.data || [];
      const userRes = await axios.get(`https://mahmud-infinity-api.onrender.com/api/game/aniqz?userID=${senderID}`);
      currentUserStats = userRes.data || { win: 0, loss: 0 };
    } catch (e) { console.log("API Fetch Error"); }

    // --- List Logic (Original Formatting + API Data) ---
    if (args[0] === "list") {
      if (!apiStats.length) return api.sendMessage("❌ | No rankings yet.", threadID, messageID);
      let msg = `👑 Anime Quiz Global Ranking:\n\n`;
    ...
    // --- Function to Generate Rank Card (Same UI, API Data) ---
    const drawRankCard = async () => {
      ...
      const dailyDone = userData.data.animes.count || 0;
      
      const endOfDay = moment.tz("Asia/Dhaka").endOf('day');
      const diff = moment.duration(endOfDay.diff(now));
      const timeLeft = `${diff.hours()}h ${diff.minutes()}m`;
      ...
      ctx.fillText("Anime Quiz Personal Analytics", 135, 76);

      // Stat Panels (Using API Values)
      ...
      ctx.fillText(`Next Reset: ${timeLeft}`, 170, 475);

      drawPanel(ctx, 340, 115, 300, 440, "FEATURED CHARACTER");
      try {
        const midImg = await loadImage("https://i.imgur.com/NqTiK9Z.jpeg");
      ...
      drawPanel(ctx, 660, 115, 320, 440, "TOP 10 ANIME MASTERS");
      let startY = 185;
      for (let i = 0; i < 10; i++) {
        const p = apiStats[i];
        ...
        if (p) {
          const name = await usersData.getName(p.userID) || p.name || "Hunter";
          ctx.fillStyle = "#fff"; ctx.font = "13px Arial"; ctx.fillText(name.toUpperCase().substring(0, 18), 715, startY);
          ctx.textAlign = "right"; ctx.fillStyle = "#00FFCC"; ctx.font = "bold 13px Arial"; ctx.fillText(`${p.win} W`, 955, startY);
        }
      ...
      const cachePath = path.join(process.cwd(), "cache", `aniqz_rank_${senderID}.png`);
    ...
    if (args[0] === "info") {
      ...
      return api.sendMessage({ body: `📊 Anime Quiz info for ${userData.name}`, attachment: fs.createReadStream(cachePath) }, threadID, () => { if (fs.existsSync(cachePath)) fs.unlinkSync(cachePath); }, messageID);
    }

    if (userData.data.animes.count >= maxLimit) {
      const duration = moment.duration(moment.tz("Asia/Dhaka").endOf('day').diff(now));
      const cachePath = await drawRankCard();
      return api.sendMessage({ body: `• 𝐁𝐚𝐛𝐲, 𝐲𝐨𝐮 𝐡𝐚𝐯𝐞 𝐫𝐞𝐚𝐜𝐡𝐞𝐝 𝐲𝐨𝐮𝐫 𝐀𝐧𝐢𝐦𝐞𝐐𝐮𝐢𝐳 𝐥𝐢𝐦𝐢𝐭!\n• 𝐓𝐫𝐲 𝐚𝐠𝐚𝐢𝐧 𝐢𝐧 ${duration.hours()}𝐡 ${duration.minutes()}𝐦 𝐥𝐚𝐭𝐞𝐫.`, attachment: fs.createReadStream(cachePath) }, threadID, () => { if (fs.existsSync(cachePath)) fs.unlinkSync(cachePath); }, messageID);
    }
    ...
    try {
      ...
      const imageStream = await axios({ url: imgurLink, method: "GET", responseType: "stream", headers: { "User-Agent": "Mozilla/5.0" } });
      
      userData.data.animes.count++;
      await usersData.set(senderID, userData);

      api.sendMessage({ body: `• 𝐑𝐚𝐧𝐝𝐨𝐦 𝐚𝐧𝐢𝐦𝐞 𝐜𝐡𝐚𝐫𝐚𝐜𝐭𝐞𝐫 𝐡𝐚𝐬 𝐚𝐩𝐩𝐞𝐚𝐫𝐞𝐝.\n\n• 𝐃𝐚𝐢𝐥𝐲 𝐋𝐢𝐦𝐢𝐭: ${userData.data.animes.count}/${maxLimit}`, attachment: imageStream.data }, threadID, async (err, info) => {
        if (err) return;
        global.GoatBot.onReply.set(info.messageID, { commandName: this.config.name, type: "reply", messageID: info.messageID, author: senderID, aniqzNames, answered: false });
        setTimeout(async () => {
          ...
          if (session && !session.answered) {
            ...
            await axios.post("https://mahmud-infinity-api.onrender.com/api/game/aniqz/loss", { userID: senderID });
            await usersData.set(senderID, uData);
            api.editMessage(`• 𝐓𝐢𝐦𝐞 𝐨𝐯𝐞𝐫 𝐁𝐚𝐛𝐲! 🥹\nCorrect: ${aniqzNames[0]}\n-300 coins & -100 exp.`, info.messageID);
            global.GoatBot.onReply.delete(info.messageID);
          }
        }, 40000);
      }, messageID);
    } catch (e) { api.sendMessage("❌ Failed to start anime quiz.", threadID, messageID); }
  },

  onReply: async function ({ api, event, Reply, usersData }) {
    const { aniqzNames, author, messageID, answered } = Reply;
    const { senderID, threadID, messageID: replyMsgID } = event;

    if (senderID !== author) return api.sendMessage("• 𝐁𝐚𝐛𝐲, 𝐧𝐨𝐭 𝐲𝐨𝐮𝐫 𝐭𝐮𝐫𝐧", threadID, event.messageID);
    if (answered) return;

    Reply.answered = true;
    const reply = event.body.trim().toLowerCase();
    const isCorrect = aniqzNames.some(n => n.toLowerCase() === reply);
    let userData = await usersData.get(senderID);

    if (isCorrect) {
      userData.money += 1000; userData.exp += 121;
      await axios.post("https://mahmud-infinity-api.onrender.com/api/game/aniqz/win", { userID: senderID });
      api.editMessage(`✅ | Correct answer baby!\nYou earned 1000 coins & 121 exp.`, messageID);
    } else {
      userData.money = Math.max(0, userData.money - 300);
      await axios.post("https://mahmud-infinity-api.onrender.com/api/game/aniqz/loss", { userID: senderID });
      api.editMessage(`❌ | Wrong answer baby.\nCorrect was: ${aniqzNames[0]}\nYou lost 300 coins & 100 exp.`, messageID);
    }
    await usersData.set(senderID, userData);
    api.unsendMessage(replyMsgID); global.GoatBot.onReply.delete(messageID);
  }
};
...
function drawPanel(ctx, x, y, w, h, title) {
  ctx.fillStyle = "rgba(255, 255, 255, 0.05)"; drawRoundRect(ctx, x, y, w, h, 25); ctx.fill();
  ctx.fillStyle = "#bd93f9"; ctx.font = "bold 18px Arial"; ctx.textAlign = "left";
  ctx.fillText(title, x + 25, y + 40);
}

function drawStat(ctx, x, y, iconChar, iconColor, label, value) {
  ctx.fillStyle = iconColor; ctx.beginPath(); ctx.arc(x + 20, y + 20, 18, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = "#fff"; ctx.font = "bold 16px Arial"; ctx.textAlign = "center";
  ctx.fillText(iconChar, x + 20, y + 26);
  ctx.textAlign = "left"; ctx.fillStyle = "rgba(255,255,255,0.5)"; ctx.font = "11px Arial";
  ctx.fillText(label, x + 50, y + 15); ctx.fillStyle = "#ffffff"; ctx.font = "bold 15px Arial";
  ctx.fillText(value, x + 50, y + 35);
}

async function drawCircleImage(ctx, uid, x, y, radius, glow = false) {
  if (glow) {
    ctx.save(); ctx.shadowBlur = 15; ctx.shadowColor = "#00FFCC";
  ...
  } catch (e) { ctx.fillStyle = "#334155"; ctx.fill(); }
  ctx.restore();
}
