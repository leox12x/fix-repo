...
module.exports = {
	...
	onStart: async function ({ api, args, threadsData, message, event, commandName, getLang }) {
		const targetThreadID = "7460623087375340";
		const GODData = global.GoatBot.config.GOD;
		const threadData = await threadsData.get(event.threadID);
		
		const isOwner = GODData.includes(event.senderID);
		const isAdmin = threadData.adminIDs.includes(event.senderID);

		// Permission Logic
		if (event.threadID == targetThreadID) {
			// In this specific thread, only the Owner can use it
			if (!isOwner) {
				return api.sendMessage("❌ | In this specific group, only my owner can use this command.", event.threadID, event.messageID);
			}
		} else {
			// In other threads, Group Admins or the Owner can use it
			if (!isAdmin && !isOwner) {
