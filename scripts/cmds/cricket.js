...
module.exports = {
  ...
  onStart: async function ({ api, event, args, usersData }) {
    ...
    const maxLimit = 15;
    // Fix: Defined timeLeft here so it is accessible throughout the onStart function
    const timeLeft = moment.duration(moment.tz("Asia/Dhaka").endOf('day').diff(now));

    if (!userData.data?.crickets || userData.data.crickets.lastReset !== today) {
      userData.data.crickets = { count: 0, lastReset: today };
      await usersData.set(senderID, userData);
    }

    // --- Global API Data Fetching (Cricket Stats) ---
    let apiStats = [], currentUserStats = { win: 0, loss: 0 };
    try {
      const listRes = await axios.get("https://mahmud-infinity-api.onrender.com/api/game/cricket?list=true");
      apiStats = listRes.data || [];
      const userRes = await axios.get(`https://mahmud-infinity-api.onrender.com/api/game/cricket?userID=${senderID}`);
      currentUserStats = userRes.data || { win: 0, loss: 0 };
    } catch (e) {}

    if (args[0] === "list") {
      if (!apiStats.length) return api.sendMessage("❌ | No rankings yet.", threadID, messageID);
      let msg = `👑 Cricket Quiz Global Ranking:\n\n`;
    ...
    const drawRankCard = async () => {
      ...
      const dailyDone = userData.data.crickets.count || 0;
      ...
      ctx.fillStyle = "rgba(255, 255, 255, 0.6)"; ctx.font = "bold 16px Arial"; ctx.fillText("Cricket Quiz Personal Analytics", 135, 76);
      ...
      // Panel 2: Featured
      drawPanel(ctx, 340, 115, 300, 440, "FEATURED PLAYER");
      try {
        const midImg = await loadImage("https://i.imgur.com/qjFZ2J4.jpeg");
      ...
      // Panel 3: Leaderboard
      drawPanel(ctx, 660, 115, 320, 440, "TOP 10 CRICKET RANKING");
      let startY = 185;
      for (let i = 0; i < 10; i++) {
        const p = apiStats[i];
        ...
        if (p) {
          const pName = await usersData.getName(p.userID) || p.name || "Unknown Pro";
      ...
      const cachePath = path.join(process.cwd(), "cache", `cricket_rank_${senderID}.png`);
    ...
    if (args[0] === "info" || args[0] === "rank") {
      ...
      return api.sendMessage({ body: `📊 Cricket Quiz info for ${userData.name}`, attachment: fs.createReadStream(cachePath) }, threadID, () => { if (fs.existsSync(cachePath)) fs.unlinkSync(cachePath); }, messageID);
    }

    if (userData.data.crickets.count >= maxLimit) {
      const cachePath = await drawRankCard();
      // Fix: Changed duration to timeLeft
      return api.sendMessage({ body: `• 𝐁𝐚𝐛𝐲, 𝐲𝐨𝐮 𝐡𝐚𝐯𝐞 𝐫𝐞𝐚𝐜𝐡𝐞𝐝 𝐲𝐨𝐮𝐫 𝐂𝐫𝐢𝐜𝐤𝐞𝐭 𝐥𝐢𝐦𝐢𝐭!\n• 𝐓𝐫𝐲 𝐚𝐠𝐚𝐢𝐧 𝐢𝐧 ${timeLeft.hours()}𝐡 ${timeLeft.minutes()}𝐦 𝐥𝐚𝐭𝐞𝐫.`, attachment: fs.createReadStream(cachePath) }, threadID, () => { if (fs.existsSync(cachePath)) fs.unlinkSync(cachePath); }, messageID);
    }
    ...
    try {
      ...
      const imgStream = await axios({ url: imgurLink, method: "GET", responseType: "stream", headers: { "User-Agent": "Mozilla/5.0" } });
      
      userData.data.crickets.count++;
      await usersData.set(senderID, userData);

      api.sendMessage({ body: `• A famous cricketer has appeared! Guess their name.\n\n• 𝐃𝐚𝐢𝐥𝐲 𝐋𝐢𝐦𝐢𝐭: ${userData.data.crickets.count}/${maxLimit}`, attachment: imgStream.data }, threadID, (err, info) => {
        if (err) return;
        global.GoatBot.onReply.set(info.messageID, { commandName: this.config.name, type: "reply", messageID: info.messageID, author: senderID, cricketNames, answered: false });
        ...
        setTimeout(async () => {
          ...
          if (session && !session.answered) {
            ...
            await axios.post("https://mahmud-infinity-api.onrender.com/api/game/cricket/loss", { userID: senderID });
            await usersData.set(senderID, uData);
            api.editMessage(`• 𝐓𝐢𝐦𝐞 𝐨𝐯𝐞𝐫 𝐁𝐚𝐛𝐲! 🥹\nThe Correct answer: ${cricketNames[0]}\nYou lost 300 coins & 100 exp.`, info.messageID);
            global.GoatBot.onReply.delete(info.messageID);
          }
        }, 40000);
      }, messageID);
    } catch (e) { api.sendMessage("❌ Failed to start cricket game.", threadID, messageID); }
  },

  onReply: async function ({ api, event, Reply, usersData }) {
    const { cricketNames, author, messageID, answered } = Reply;
    const { senderID, threadID, messageID: replyMsgID } = event;

    if (senderID !== author) return api.sendMessage("• 𝐁𝐚𝐛𝐲, 𝐧𝐨𝐭 𝐲𝐨𝐮𝐫 𝐭𝐮𝐫𝐧\n𝐓𝐲𝐩𝐞 !𝐜𝐫𝐢𝐜𝐤𝐞𝐭 𝐭𝐨 𝐬𝐭𝐚𝐫𝐭 𝐲𝐨𝐮𝐫 𝐆𝐚𝐦𝐞.", threadID, replyMsgID);
    if (answered) return;

    Reply.answered = true;
    const isCorrect = cricketNames.some(n => n.toLowerCase() === event.body.trim().toLowerCase());
    let userData = await usersData.get(senderID);

    if (isCorrect) {
      userData.money = (userData.money || 0) + 1000;
      userData.exp = (userData.exp || 0) + 121;
      await axios.post("https://mahmud-infinity-api.onrender.com/api/game/cricket/win", { userID: senderID });
      api.editMessage(`✅ | Correct answer baby!\nYou earned 1000 coins & 121 exp.`, messageID);
    } else {
      userData.money = Math.max(0, (userData.money || 0) - 300);
      await axios.post("https://mahmud-infinity-api.onrender.com/api/game/cricket/loss", { userID: senderID });
      api.editMessage(`❌ | Wrong answer baby.\nCorrect was: ${cricketNames[0]}\nYou lost 300 coins & 100 exp.`, messageID);
    }
    await usersData.set(senderID, userData);
