...
module.exports = {
  ...
  onStart: async function ({ event, api, args }) {
    ...
    if (mention.length === 1) {
      one = senderID; // Assign sender to `one`
      two = mention[0]; // Assign the mentioned user to `two`
    } else {
      // If two people are mentioned, assign them to `one` and `two`
      one = mention[0];
      two = mention[1];
    }

    try {
      // Generate the image with the two mentioned users
