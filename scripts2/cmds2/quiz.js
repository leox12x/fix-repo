const axios = require("axios");
const moment = require("moment-timezone");

const baseApiUrl = async () => {
  try {
    const base = await axios.get("https://raw.githubusercontent.com/mahmudx7/HINATA/main/baseApiUrl.json");
    return base.data.mahmudexe;
  } catch (e) { return null; }
};

module.exports = {
  config: {
    name: "quiz",
    aliases: ["qz"],
    version: "15.0",
    author: "MahMUD",
    countDown: 20,
    role: 0,
    category: "game",
    guide: {
      en: "{pn} — random quiz\n{pn} list — show global leaderboard\n{pn} info — show your global stats\n{pn} category — show categories"
    }
  },

  onStart: async function ({ api, args, event, usersData }) {
    const { senderID, threadID, messageID } = event;
    let userData = await usersData.get(senderID);
    const now = moment.tz("Asia/Dhaka");
    const today = now.format("DD/MM/YYYY");
    const maxLimit = 15;

    if (!userData.data.quizs || userData.data.quizs.lastReset !== today) {
      userData.data.quizs = { count: 0, lastReset: today };
      await usersData.set(senderID, userData);
    }

    // --- Global API Data Fetching (Quiz Stats) ---
    let apiStats = [], currentUserStats = { win: 0, loss: 0 };
    try {
      const listRes = await axios.get("https://mahmud-infinity-api.onrender.com/api/game/quiz?list=true");
      apiStats = listRes.data || [];
      const userRes = await axios.get(`https://mahmud-infinity-api.onrender.com/api/game/quiz?userID=${senderID}`);
      currentUserStats = userRes.data || { win: 0, loss: 0 };
    } catch (e) {}

    // --- Leaderboard List ---
    if (args[0] === "list") {
      if (!apiStats.length) return api.sendMessage("❌ | No rankings yet.", threadID, messageID);
      let msg = `👑 Quiz Game Global Ranking:\n\n`;
      for (let i = 0; i < Math.min(apiStats.length, 100); i++) {
        const u = apiStats[i];
        const name = await usersData.getName(u.userID) || u.name || `User ${u.userID}`;
        msg += `${i + 1}. ${name}: ${u.win} Wins\n`;
      }
      return api.sendMessage(msg, threadID, messageID);
    }

    // --- Personal Info/Rank (Text Based) ---
    if (args[0] === "info" || args[0] === "rank") {
      const index = apiStats.findIndex(u => u.userID == senderID);
      const wins = currentUserStats.win || 0;
      const lost = currentUserStats.loss || 0;
      const rank = index === -1 ? "N/A" : index + 1;
      const dailyDone = userData.data.quizs.count || 0;
      
      let infoMsg = `📊 𝐐𝐮𝐢𝐳 𝐒𝐭𝐚𝐭𝐢𝐬𝐭𝐢𝐜𝐬 𝐟𝐨𝐫 ${userData.name}\n${"━".repeat(20)}\n`;
      infoMsg += `• Global Rank: #${rank}\n`;
      infoMsg += `• Total Wins: ${wins}\n`;
      infoMsg += `• Total Lost: ${lost}\n`;
      infoMsg += `• Daily Progress: ${dailyDone}/${maxLimit}\n`;
      infoMsg += `${"━".repeat(20)}\n© MahMUD - Global Gaming System`;
      return api.sendMessage(infoMsg, threadID, messageID);
    }

    try {
      const apiUrl = await baseApiUrl();
      if (!apiUrl) return api.sendMessage("❌ API URL not found!", threadID, messageID);

      const catRes = await axios.get(`${apiUrl}/api/quiz2/categories`);
      const validCategories = catRes.data.categories;

      if (args[0] === "category") {
        let catMsg = `>🎀\n𝐀𝐯𝐚𝐢𝐥𝐚𝐛𝐥𝐞 𝐐𝐮𝐢𝐳 𝐂𝐚𝐭𝐞𝐠𝐨𝐫𝐢𝐞𝐬:\n\n`;
        validCategories.forEach((cat, index) => {
          catMsg += `${index + 1}. ${cat.charAt(0).toUpperCase() + cat.slice(1)}\n`;
        });
        catMsg += `\n• 𝐓𝐲𝐩𝐞: !quiz [name] [bn/en]`;
        return api.sendMessage(catMsg, threadID, messageID);
      }

      if (userData.data.quizs.count >= maxLimit) {
        const duration = moment.duration(moment.tz("Asia/Dhaka").endOf('day').diff(now));
        return api.sendMessage(`• 𝐁𝐚𝐛𝐲, 𝐐𝐮𝐢𝐳 𝐥𝐢𝐦𝐢𝐭 𝐫𝐞𝐚𝐜𝐡𝐞𝐝, 𝐭𝐫𝐲 𝐚𝐠𝐚𝐢𝐧 in ${duration.hours()}h ${duration.minutes()}m.`, threadID, messageID);
      }

      let category = args[0]?.toLowerCase();
      let lang = args[1]?.toLowerCase() === "en" ? "english" : "bangla";

      if (!category) {
        category = validCategories[Math.floor(Math.random() * validCategories.length)];
      } else if (!validCategories.includes(category)) {
        return api.sendMessage(`❌ | Wrong Category!\nAvailable: ${validCategories.join(", ")}`, threadID, messageID);
      }

      const res = await axios.get(`${apiUrl}/api/quiz2?category=${category}&lang=${lang}`);
      const quiz = res.data.data; 
      const { question, options, correctAnswer } = quiz;

      const quizMsg = `\n╭──✦ ${question}\n├‣ 𝗔) ${options.a}\n├‣ 𝗕) ${options.b}\n├‣ 𝗖) ${options.c}\n├‣ 𝗗) ${options.d}\n╰──────────────────‣\n• 𝐂𝐚𝐭𝐞𝐠𝐨𝐫𝐲: ${category}\n• 𝐑𝐞𝐩𝐥𝐲 𝐰𝐢𝐭𝐡 𝐲𝐨𝐮𝐫 𝐚𝐧𝐬𝐰𝐞𝐫.`;

      api.sendMessage(quizMsg, threadID, async (err, info) => {
        if (err) return;
        userData.data.quizs.count++;
        await usersData.set(senderID, userData);

        global.GoatBot.onReply.set(info.messageID, {
          commandName: this.config.name,
          author: senderID,
          correctAnswer,
          category,
          answered: false
        });

        setTimeout(async () => {
          const session = global.GoatBot.onReply.get(info.messageID);
          if (session && !session.answered) {
            let uData = await usersData.get(senderID);
            uData.money = Math.max(0, (uData.money || 0) - 300);
            await axios.post("https://mahmud-infinity-api.onrender.com/api/game/quiz/loss", { userID: senderID });
            await usersData.set(senderID, uData);
            api.editMessage(`• 𝐓𝐢𝐦𝐞 𝐨𝐯𝐞𝐫 𝐁𝐚𝐛𝐲! 🥹\nCorrect Answer: ${correctAnswer.toUpperCase()}\nYou lost 300 coins.`, info.messageID);
            global.GoatBot.onReply.delete(info.messageID);
          }
        }, 40000); 
      }, messageID);

    } catch (e) { return api.sendMessage("❌ Error fetching quiz data.", threadID, messageID); }
  },

  onReply: async function ({ api, event, Reply, usersData }) {
    const { senderID, threadID, messageID: replyMsgID, body } = event;
    const { correctAnswer, author, answered, category } = Reply;
    const gameMessageID = event.messageReply.messageID;

    if (senderID !== author || answered) return;
    
    const userAnswer = body?.trim()?.toLowerCase();
    if (!["a", "b", "c", "d"].includes(userAnswer)) return;

    Reply.answered = true; 
    let userData = await usersData.get(senderID);

    if (userAnswer === correctAnswer.toLowerCase()) {
      userData.money = (userData.money || 0) + 1000;
      userData.exp = (userData.exp || 0) + 121;
      await axios.post("https://mahmud-infinity-api.onrender.com/api/game/quiz/win", { userID: senderID });
      api.editMessage(`✅ | Correct answer baby!\n•You Earned 1000 coins & 121 exp.`, gameMessageID);
    } else {
      userData.money = Math.max(0, (userData.money || 0) - 300);
      userData.exp = (userData.exp || 0) - 121;
      await axios.post("https://mahmud-infinity-api.onrender.com/api/game/quiz/loss", { userID: senderID });
      api.editMessage(`❌ | Wrong answer baby!\n•The Correct Answer: ${correctAnswer.toUpperCase()}\n•You lost 300 coins & 121 exp.`, gameMessageID);
    }
    
    await usersData.set(senderID, userData);
    api.unsendMessage(replyMsgID);
    global.GoatBot.onReply.delete(gameMessageID);
  }
};
